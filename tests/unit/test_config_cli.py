...
class TestConfigSet:
    ...
    def test_set_hot_key(self, mock_requires_restart, mock_get_loader):
        ...
        with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
            ...
            mock_loader.update_config.assert_called_once_with('llms.llm.temperature', 0.8)
    ...
    def test_set_type_conversion(self, mock_get_loader):
        ...
        handle_config_set(args)
        mock_loader.update_config.assert_called_with('agents.default.enable_browsing', True)
...
class TestConfigValidate:
    ...
    def test_validate_current_config_invalid(self, mock_get_loader):
        ...
        mock_loader = MagicMock()
        mock_loader.validate_config.return_value = (False, ['Error message'], ['Warning'])
        mock_get_loader.return_value = mock_loader
    ...
    def test_validate_file_toml(self, temp_config_file):
        ...
        with patch('openhands.cli.config_commands.get_config_loader') as mock_get_loader:
            mock_loader = MagicMock()
    ...
    def test_diagnostics_healthy_config(self, mock_get_loader):
        ...
        mock_diagnostics = {
            ...
            'source_analysis': {
                ...
                'user': {'status': 'active', 'keys_count': 2, 'path': '/path/config.toml'}
            },
            'key_analysis': {
    ...
    def test_cli_workflow(self, temp_config_file):
        ...
        with patch('openhands.cli.config_commands.get_config_loader') as mock_get_loader:
            ...
            mock_loader.get_diagnostics.return_value = {
                'config_health': {'status': 'healthy', 'errors': [], 'warnings': [], 'requires_restart': False},
                'source_analysis': {},
                'key_analysis': {'cold_keys': {'keys': []}, 'hot_keys': {'keys': []}},
                'environment_analysis': {'openhands_env_vars': {}, 'env_overrides': {}, 'cli_overrides': {}},
                'recommendations': []
            }
            mock_get_loader.return_value = mock_loader
            ...
            with patch('openhands.cli.config_commands.get_config') as mock_get_config:
                ...
                with patch('openhands.cli.config_commands.requires_restart', return_value=False):
                    # Test show command
                    args = MockArgs(format='json', sources=False)
                    with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                        handle_config_show(args)
