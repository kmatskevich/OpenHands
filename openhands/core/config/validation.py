...
class ConfigValidator:
    ...
    def validate(self, config_data: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
        ...
        try:
            ...
            config = OpenHandsConfig(**config_data)
    ...
    def _validate_llm_config(self, llm_config: Dict[str, Any]) -> None:
        ...
        if model and not api_key and not base_url:
            if any(provider in model.lower() for provider in ['openai', 'gpt', 'anthropic', 'claude']):
    ...
    def _validate_runtime_config(self, runtime: str) -> None:
        ...
        valid_runtimes = ['docker', 'local', 'remote', 'e2b', 'modal']
        if runtime not in valid_runtimes:
            self.errors.append(f'Invalid runtime: {runtime}. Must be one of: {valid_runtimes}')
        ...
        for agent_name, agent_config in agents_config.items():
            ...
            if memory_config:
                ...
                if max_threads is not None and max_threads <= 0:
                    self.errors.append(f'Agent {agent_name}: max_threads must be positive')
                
                condenser_config = memory_config.get('condenser', {})
                if condenser_config:
                    condenser_type = condenser_config.get('type')
                    if condenser_type and condenser_type not in ['truncation', 'summary']:
                        self.warnings.append(f'Agent {agent_name}: unknown condenser type: {condenser_type}')
    ...
    def _validate_security_config(self, security_config: Dict[str, Any]) -> None:
        ...
        if sandbox_mode not in ['strict', 'permissive', 'disabled']:
            self.errors.append('Security sandbox_mode must be: strict, permissive, or disabled')
        ...
        def check_deprecated(data: Dict[str, Any], prefix: str = '') -> None:
            for key, value in data.items():
                ...
                if key in deprecated_keys:
                    self.warnings.append(f'Deprecated key "{full_key}": {deprecated_keys[key]}')
        ...
        try:
            ...
            if llm_config:
                ...
                if not api_key and not base_url and model:
                    if any(provider in model.lower() for provider in ['openai', 'gpt', 'anthropic', 'claude']):
                        recommendations.append('Consider setting an API key for your LLM model')
            
            # Check runtime configuration
            runtime = config_dict.get('runtime', 'docker')
            if runtime == 'local':
                recommendations.append('Local runtime is less secure - consider using Docker runtime')
            
            # Check security settings
            security_config = config_dict.get('security', {})
            if security_config.get('sandbox_mode') == 'disabled':
                recommendations.append('Security sandbox is disabled - enable for better security')
            
            # Check for restart requirements
            if self.loader.requires_restart():
                recommendations.append('Configuration changes require restart to take effect')
            
            # Check for environment overrides
            if self.loader._env_overrides:
                recommendations.append('Environment variables are overriding configuration - ensure this is intentional')
