...
from openhands.core.config.config_utils import load_from_env, load_from_toml
    ...
    try:
        ...
        if config.runtime == 'local':
            ...
            if not os.path.exists(workspace_base):
                errors.append(f"Project root directory does not exist: {workspace_base}")
            elif not os.access(workspace_base, os.R_OK | os.W_OK):
                errors.append(f"Project root directory is not readable/writable: {workspace_base}")
...
async def get_diagnostics() -> DiagnosticsResponse:
    ...
    try:
        ...
        config_info = {
            ...
            'workspace_mount_path_in_sandbox': getattr(config, 'workspace_mount_path_in_sandbox', '/workspace'),
        }
        ...
        system_info = {
            ...
            'env_vars': {
                ...
                if key.startswith('OPENHANDS_') or key in ['DOCKER_HOST', 'HOME', 'USER']
            }
        }
